...
#include <string.h>
 ...
 if the user puts include on the command line it will include the end date*/
...
int main ( int argc, char *argv[] ) {
  ...
  if (argc < 7) {
      ...
      printf ( "Usage: ./StageA dd1 mm1 yyyy1  dd2 mm2 yyyy2 (include) \n" );
       exit ( 1 );
    } else {
       dd1 = atoi ( argv[1] );
       mm1 = atoi ( argv[2] );
 ...
 if (num < 0) {
     ...
     /* check to see if include is on the command line */
     if (argc == 8 ) {
        if( strcmp("include", argv[7] ) == 0 && (num > 0) ) {

         num = ((Dayyear[mm2-1]+ (dd2)) - (Dayyear[mm1-1]+(dd1-1)));
         printf ("%d\n", num);
      }
